clear all
% close all
% load testing_label_target_clutter_H1_SNR-4.mat % Τtarget
% load testing_clutter_H1_SNR-4.mat % ⊿Τ竒筁truncated⊿Τclutter
% noise_Map_Deep_CFAR_unshaped = csvread('testing_DLCFAR_noise.csv');

load testing_noT_label_target_clutter_CNR20_20_H4_SNR-4.mat % Τtarget
load testing_noT_clutter_CNR20_20_H4_SNR-4.mat % ⊿Τ竒筁truncated⊿Τclutter
noise_Map_Deep_CFAR_unshaped = csvread('testing_DLCFAR_noise.csv');
% load para.mat
% load yolo_roc.mat
N=16;M=16;
...
L = 6; % L = window -1
Num_Padding = L/2;
for iii = 1:size(RD_map_clutter,3) % 家览Ω计
    tempMatrix = [fliplr(RD_map_clutter(:,end-Num_Padding+1:end,iii)) RD_map_clutter(:,:,iii) fliplr(RD_map_clutter(:,1:Num_Padding,iii))];
...
for iiii = 1:length(P_FA_set)
    ...
    for TimeIndex = 1:size(RD_map_clutter,3)
        ...
        %单T碞块1T块0
        DetectionMap_DLCFAR = double(RD_map_clutter(:,:,TimeIndex) >= T_DeepCFAR);
        DetectionMap_CACFAR = double(RD_map_clutter(:,:,TimeIndex) >= T_CACFAR);        
        DetectionMap_GOCACFAR = double(RD_map_clutter(:,:,TimeIndex) >= T_GOCACFAR);        
        DetectionMap_SOCACFAR = double(RD_map_clutter(:,:,TimeIndex) >= T_SOCACFAR);       
        DetectionMap_OSCFAR_5 = double(RD_map_clutter(:,:,TimeIndex) >= T_OSCFAR_5);
        DetectionMap_OSCFAR_10 = double(RD_map_clutter(:,:,TimeIndex) >= T_OSCFAR_10);
        DetectionMap_OSCFAR_20 = double(RD_map_clutter(:,:,TimeIndex) >= T_OSCFAR_20);
        DetectionMap_OSCFAR_30 = double(RD_map_clutter(:,:,TimeIndex) >= T_OSCFAR_30);    
...
% save SNR20dB_clutter_CNR20_H1_cfar_roc PDR_DeepCFAR FAR_DeepCFAR PDR_CACFAR PDR_GOCACFAR PDR_SOCACFAR PDR_OSCFAR_5 PDR_OSCFAR_10 PDR_OSCFAR_20 PDR_OSCFAR_30 FAR_CACFAR FAR_GOCACFAR FAR_SOCACFAR FAR_OSCFAR_5 FAR_OSCFAR_10 FAR_OSCFAR_20 FAR_OSCFAR_30
% open('SNR20dB_clutter_CNR20_H1_w6(clutter)_DNNw5_200000_soft.fig');
...
% figure()
% mesh(RD_map_clutter,'edgecolor','k');
% 
% figure()
% mesh(RD_map_clutter,'edgecolor','k');
% hold on
